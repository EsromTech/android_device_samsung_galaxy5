...
final class AttnHeadset implements Preference.OnPreferenceClickListener {
	...
	AttnHeadset(DeviceSettings arg0) {
	}

	public final boolean onPreferenceClick(final Preference preference) {

		String currentSetting = Command.getprop(DeviceSettings.PROP_HEADSET_ATTN);
		if (currentSetting.length() == 1) {
			selected = Integer.parseInt(currentSetting);
		}

		AlertDialog.Builder dialog = new AlertDialog.Builder(context)
				.setTitle(context.getText(R.string.attn_headset))
				.setSingleChoiceItems(DeviceSettings.attn, selected, new OnClickListener() {

					public void onClick(DialogInterface arg0, int arg1) {

						SystemProperties.set(DeviceSettings.PROP_HEADSET_ATTN, ""
								+ arg1);
